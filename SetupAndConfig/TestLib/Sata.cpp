...
static int wcache   = 0,set_wcache   = 0, get_wcache   = 0;
...
//           strcpy(arg, argv[0]);
	   ...
	   std::cout << fd << std::endl;  
     //      std::cout << "SataStorage Init" << std::endl;  
            return ErrorCode;
        }
  ...
  perror:
	 ...
	 exit(errno);
        ...
        virtual int Identify(void)
        {
            ...
            std::cout << "NvmeStorage Identify" << std::endl;         
...
extern "C" {
    ...
    void SataStorage_Init(FrameWorkSata* FrameWorkSata, char *ptr){ 
	//	char *ptr="/dev/sda";
//           char *ptr1=NULL; 
           FrameWorkSata->Init(&ptr); }
    void SataStorage_Identify(FrameWorkSata* FrameWorkSata){ 
		char *ptr="/dev/sda";
//           char *ptr1=NULL; 
           FrameWorkSata->Identify(); }
 void ReadSector(FrameWorkSata* FrameWorkSata, __u64 val){ 
		char *ptr="/dev/sda";
//           char *ptr1=NULL; 
           FrameWorkSata->ReadSector(val); }

void WriteSector(FrameWorkSata* FrameWorkSata, __u64 val){ 
		char *ptr="/dev/sda";
//           char *ptr1=NULL; 
           FrameWorkSata->WriteSector(val); }
void WriteCache(FrameWorkSata* FrameWorkSata, int val){ 
		char *ptr="/dev/sda";
//           char *ptr1=NULL; 
           FrameWorkSata->WriteCache(val); }
...
extern "C" {
	...
	void NvmeStorage_Identify(NvmeStorage* NvmeStorage1){
		char *ptr="/dev/nvme0";NvmeStorage1->nvme_identify_ctrl(&NvmeStorage1->ctrl);
		NvmeStorage1->show_nvme_id_ctrl_(&NvmeStorage1->ctrl,0);
			}
	void NvmeStorage_list(NvmeStorage* NvmeStorage1){
		char *ptr="/dev/nvme0";NvmeStorage1->nvme_list();
		//NvmeStorage1->show_nvme_id_ctrl_(&NvmeStorage1->ctrl,0);
			}	
}
...
//if (argc < 2) {
	...
	printf("fd value before open_dev %d\n", ::fd);




	printf("fd value after open_dev %d\n", ::fd);
